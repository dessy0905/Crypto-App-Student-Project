...
const FilterDropdown = ({
	...
	return (
		<div ref={ref} className="relative">
			...
			{/* Dropdown panel */}
			{isOpen && (
				<div className="absolute top-full left-0 mt-2 z-50 bg-white rounded-xl border border-gray-200 shadow-elevation-2 min-w-[180px] max-h-[320px] overflow-hidden flex flex-col animate-in fade-in">
					{/* Search input */}
					{searchable && (
						<div className="p-3 border-b border-gray-200">
							...
							<input
								...
								autoFocus
								className="w-full pl-9 pr-3 py-2 rounded-lg border border-gray-300 bg-white text-body text-gray-900 outline-none focus:border-blue-600 transition-colors placeholder:text-gray-400"
								/>
							</div>
						</div>
					)}
					...
					<div className="overflow-y-auto py-1">
						{filtered.length === 0 ? (
							<p className="px-4 py-3 text-label-2 text-gray-400">No results</p>
						) : (
							filtered.map((opt) => {
								...
								return (
									...
									`}
								>
									{opt.icon && <span className="text-gray-600 shrink-0">{opt.icon}</span>}
									<div className="flex flex-col min-w-0 flex-1">
										<span className={`text-body truncate ${isSelected ? 'text-gray-900 font-semibold' : 'text-gray-900'}`}>
											{opt.label}
										</span>
										{opt.sublabel && (
											<span className="text-legal text-gray-400 truncate">{opt.sublabel}</span>
											)}
										</div>
