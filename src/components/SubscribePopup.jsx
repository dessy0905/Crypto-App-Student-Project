				...
				<div className="w-full flex-1 p-8 text-center flex flex-col justify-center">
					{subscribed ? (
						<div className="flex flex-col items-center justify-center space-y-4 transition-opacity duration-500">
							...
							<h3 className="text-3xl font-display font-medium text-green-600">You have subscribed</h3>
							<p className="text-gray-500 mt-2">Thanks for subscribing to Coinbase Bytes.</p>
							<button
								onClick={onClose}
								className="mt-6 w-full rounded-full bg-blue-600 px-8 py-3 font-semibold text-white transition-colors hover:bg-blue-700"
							>
								Close
							</button>
						</div>
						...
						<div className="flex flex-col items-center transition-opacity duration-500">
							...
							<form onSubmit={handleSubmit} className="flex flex-col w-full space-y-4">
								<input
									...
									required
									className="w-full rounded-xl border border-gray-300 bg-white px-4 py-3 text-base text-black placeholder:text-gray-400 focus:border-blue-600 focus:outline-none focus:ring-1 focus:ring-blue-600 text-center"
								/>
								<button
									type="submit"
									className="w-full rounded-full bg-blue-600 px-6 py-3 font-semibold text-white transition-colors hover:bg-blue-700"
								>
									Subscribe
								</button>
							</form>
							<p className="mt-6 text-xs text-gray-500 leading-relaxed">
								By submitting your email address, you are signing up to receive Coinbase Bytes communications to your inbox. For information on how we use your data see <a href="#" className="text-blue-600 hover:underline">www.coinbase.com/legal/privacy</a>.
							</p>
						</div>
					)}
				</div>
